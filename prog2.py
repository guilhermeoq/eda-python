...
def analysis():
    ...
    while i < len(intervals):
        ...
        for j in range(len(energies)):
            if (energies[j][0] < intervals[i]) and (energies[j][0] >= intervals[i-1]):
                count+=1
        #print("Interval: "+str(intervals[i-1])+" - quit()"+str(intervals[i])+ "; Count: "+str(count))
        f.write(str(intervals[i-1])+" - "+str(intervals[i])+";"+str(count)+"\n")
        i+=1
    f.write("end\n")
